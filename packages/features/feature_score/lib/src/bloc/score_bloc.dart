...
class ScoreBloc extends Bloc<ScoreEvent, ScoreState> {
  ...
  ScoreBloc({
    ...
    on<LayerVisibilityToggled>(_onLayerVisibilityToggled);
    on<CleanWorkspaceToggled>(_onCleanWorkspaceToggled);
    on<PenColorSelected>(_onPenColorSelected);
  ...
  void _onAnnotationsUpdated(
    ...
    final teacherLayer = event.annotations.layers.where(
      (layer) => layer.ownerId == piece.teacherId,
    );
    final studentId = piece.studentId;
    final studentLayer = event.annotations.layers.where(
      (layer) => studentId != null && layer.ownerId == studentId,
    );
    emit(
      state.copyWith(
        teacherStrokes: teacherLayer.isEmpty
            ? const []
            : teacherLayer.first.strokes,
        studentStrokes: studentLayer.isEmpty
            ? const []
            : studentLayer.first.strokes,
  ...
  void _onLayerVisibilityToggled(
    LayerVisibilityToggled event,
    Emitter<ScoreState> emit,
  ) {
    switch (event.layer) {
      case LayerKind.teacherInk:
        emit(state.copyWith(teacherInkVisible: !state.teacherInkVisible));
      case LayerKind.studentInk:
        emit(state.copyWith(studentInkVisible: !state.studentInkVisible));
      case LayerKind.audioPins:
  ...
  void _onPenColorSelected(PenColorSelected event, Emitter<ScoreState> emit) {
    emit(state.copyWith(selectedColorId: event.colorId, eraserActive: false));
  }
  ...
  Future<void> _onStrokeCompleted(
    ...
    final stroke = InkStroke(
      ...
      colorId: inkColorIdFor(state.selectedColorId),
      points: event.points,
    );
    // Pushed onto the session-local undo stack optimistically; the
    // authoritative stroke lists (teacher/studentStrokes) only update once
    // the repository's stream re-emits, avoiding a second source of truth.
  ...
  Future<void> _onStrokeErased(
    ...
    final piece = state.piece;
    if (piece == null) return;
    final ownStrokes = state.currentRole == PieceRole.teacher
        ? state.teacherStrokes
        : state.studentStrokes;
    // The UI should never surface an erase gesture for the other party's
